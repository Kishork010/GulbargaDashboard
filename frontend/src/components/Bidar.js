...
function PravinTalvar() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Filter rows where TL Name matches PPT_300B640 (Pravin Talvar)
      const tlData = json.filter(row => row['tl_name']?.trim() === 'PPT_300B640');
      ...
      // Status counts based on Sales Stage
      ...
      // Model live leads excluding EV models
      const modelCounts = {};
      liveData.forEach(row => {
...
const currentMonthData = tlData
  ...
  return (
    <div className="gulbarga-container">
      <h2>PRAVIN TALVAR(PPT_300B640) Bidar</h2>
