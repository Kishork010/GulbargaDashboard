...
function GulbargaCombined() {
  ...
  const fetchData = async () => {
    try {
      ...
      const combinedData = json.filter(row => {
        const tl = row['tl_name']?.trim();
        return tl === 'PVM_300B640' || tl === 'PSS1_300B640';
      });
      setTlData(combinedData); 


      const allTotal = combinedData.length;
      const allHot = combinedData.filter(r => r['lead_classification']?.trim().toLowerCase() === 'hot').length;
      const allWarm = combinedData.filter(r => r['lead_classification']?.trim().toLowerCase() === 'warm').length;
      const allCold = combinedData.filter(r => r['lead_classification']?.trim().toLowerCase() === 'cold').length;

      const showroomWalkin = combinedData.filter(r =>
      ...
      const liveCombinedData = combinedData.filter(row => liveStages.includes(row['sales_stage']?.trim()));

      const lobCounts = { Cars: 0, UVs: 0, EVBU: 0 };
      liveCombinedData.forEach(row => {
      ...
      console.error('Error fetching combined TL data:', err);
    }
  };
  ...
  return (
    <div className="gulbarga-container">
      <h2>VIJAYKUMAR MALIPATIL & SURAJ SARDA (Gulbarga)</h2>
  ...
  <div style={{ overflowX: 'auto' }}>
    ...
    <tbody> 
      {currentMonthData.map((row, index) => (
        <tr key={index}>
          <td className="py-2 px-4 border-b">{row['stage_1_date']}</td>
          <td className="py-2 px-4 border-b">{row['first_name']}</td>
          <td className="py-2 px-4 border-b">{row['mobile_number']}</td>
          <td className="py-2 px-4 border-b">{row['parent_product_line']}</td>
          <td className="py-2 px-4 border-b">{row['sales_stage']}</td>
          <td className="py-2 px-4 border-b">{row['source_of_contact']}</td>
        </tr>
      ))}
    </tbody>
  </table>
...
export default GulbargaCombined;
